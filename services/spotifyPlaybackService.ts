...
class SpotifyPlaybackService {
   ...
   * Returns true if the device becomes active, false otherwise.
   */
  private async handshakeDevice(targetDeviceId: string, maxAttempts: number = 8): Promise<boolean> {
    ...
    // 1. Force a transfer with play:false to anchor the session without starting music
    try {
  ...
  async playUrisWithRetry(uris: string[], targetDeviceId?: string, offsetIndex?: number): Promise<void> {
    ...
    const executePlay = async (deviceId: string): Promise<void> => {
      ...
      // Use silent: true so we can handle the 404/204 state locally
    ...
    // Step 1: Initial handshake for podcasts
    if (isPodcast) {
    ...
    while (attempt < 4) {
      try {
        ...
        // Step 2: Handle the specific Podcast Inactive Player error (404 Not Found)
