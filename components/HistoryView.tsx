...
// Singleton for Apple Music library writes
...
// Derive mood label from saved rulesSnapshot
const moodLabel = (record.rulesSnapshot.moodLevel ?? 0.5) < MOOD_ZONES.ZEN_MAX
? ‘Zen’
: (record.rulesSnapshot.moodLevel ?? 0.5) < MOOD_ZONES.FOCUS_MAX
? ‘Focus’
: ‘Chaos’;
...
- handlePlayRecord — plays the saved track list directly through Apple Music.
- No device picker needed — MusicKit always plays on the current device.
  */
...
- handleSaveToAppleMusic — creates a real playlist in Apple Music library
- from a saved Vault record.
  */
...
try {
  ...
  const moodLabel = (record.rulesSnapshot.moodLevel ?? 0.5) < MOOD_ZONES.ZEN_MAX
    ? 'Zen' : (record.rulesSnapshot.moodLevel ?? 0.5) < MOOD_ZONES.FOCUS_MAX ? 'Focus' : 'Chaos';
  const description = `Mode: ${record.optionName} | Mood: ${moodLabel} | Explore: ${Math.round(record.rulesSnapshot.discoverLevel * 100)}% | From Vault: ${record.timestamp}`;
...
// ── Sub-view: viewing a record in RunView ──
      ...
      >
        ...
        <div className="flex flex-col gap-3">
          <button
            ...
            className="relative overflow-hidden w-full bg-gradient-to-br from-palette-teal to-[#40D9D0] text-white font-black py-5 rounded-[24px] active:scale-95 transition-all uppercase tracking-widest text-xs shadow-xl disabled:opacity-50"
          >
